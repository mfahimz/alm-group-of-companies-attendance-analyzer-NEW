...
const calculateEmployeeSalary = (input) => {
    ...
    const {
        isAlMaraghi = false, divisor = 30, otDivisor = 30, prevMonthDivisor = 30,
        otNormalRate = 1.25, otSpecialRate = 1.5, wpsCapEnabled = false, wpsCapAmount = 4900,
        balanceRoundingRule = 'EXACT', leavePayFormula = 'TOTAL_SALARY', salaryLeaveFormula = 'BASIC_PLUS_ALLOWANCES'
    } = settings || {};
    ...
    const {
        fullAbsenceCount = 0, annualLeaveCount = 0, deductibleMinutes = 0, otherMinutes = 0,
        ramadanGiftMinutes = 0, graceMinutes = 15, salaryLeaveDays: rawSalaryLeaveDays = 0,
    } = attendance || {};
    ...
    const rawTotal = totalSalary + bonus + effectiveOtOrIncentive + openLeaveSalary + variableSalary
    ...
    return {
        basic_salary: basicSalary, allowances, allowances_with_bonus: allowancesWithBonus,
        total_salary: totalSalary, working_hours: workingHours,
        working_days: attendance?.workingDays || 0, present_days: attendance?.presentDays || 0,
        full_absence_count: fullAbsenceCount, annual_leave_count: annualLeaveCount,
        sick_leave_count: attendance?.sickLeaveCount || 0, late_minutes: attendance?.lateMinutes || 0,
        early_checkout_minutes: attendance?.earlyCheckoutMinutes || 0, other_minutes: attendance?.otherMinutes || 0,
        approved_minutes: attendance?.approvedMinutes || 0, grace_minutes: graceMinutes,
        deductible_minutes: finalizedDeductibleMinutes, ramadan_gift_minutes: ramadanGiftMinutes || 0,
        salary_divisor: divisor, ot_divisor: otDivisor, prev_month_divisor: prevMonthDivisor,
        salary_leave_days: salaryLeaveDays, leaveDays, leavePay, salaryLeaveAmount, netDeduction,
        deductibleHours, deductibleHoursPay, normalOtHours, normalOtSalary, specialOtHours,
        specialOtSalary, totalOtSalary, bonus, incentive, open_leave_salary: openLeaveSalary,
        variable_salary: variableSalary, otherDeduction, advanceSalaryDeduction,
        total, wpsPay, balance, wps_cap_enabled: wpsCapEnabled, wps_cap_amount: wpsCapAmount, wps_cap_applied: wpsCapApplied
    };
};
...
Deno.serve(async (req: Request) => {
    try {
        ...
        const otDivisor = snapshot.ot_divisor || project.ot_calculation_days || divisor;
        // DIVISOR_PREV_MONTH: Read from the stored snapshot value (auto-derived at finalization time,
        // e.g. 31 for March, 28 for February). Never re-derive from project — the snapshot is the source of truth.
        const prevMonthDivisor = snapshot.prev_month_divisor || otDivisor;
        ...
        const updatePayload = {
            ...
            // Previous month calculations — preserve stored snapshot values (auto-derived at finalization).
            // These are calculated once at finalization using the correct prevMonthDivisor and must not be zeroed.
            extra_prev_month_lop_pay: snapshot.extra_prev_month_lop_pay || 0,
            extra_prev_month_deductible_hours_pay: snapshot.extra_prev_month_deductible_hours_pay || 0,
