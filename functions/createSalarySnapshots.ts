...
Deno.serve(async (req) => {
    try {
        ...
        for (const emp of employeesToProcess) {
            ...
            const annualLeaveDaysForSalary = hasConfiguredOverride
                ? salaryLeaveDaysOverride
                : calculated.annualLeaveCount;
            let salaryLeaveDays = annualLeaveDaysForSalary;

            // Calculate derived salary values - ALL rounded to 2 decimal places
            const leaveDays = annualLeaveDaysForSalary + calculated.fullAbsenceCount;
