/*
 * Calendar Based Equivalent of createSalarySnapshots
 ...
 * - employee.company === period.company
...
Deno.serve(async (req) => {
    try {
        ...
        const { calendar_period_id, report_run_id, batch_mode = false, batch_start = 0, batch_size = 10 } = await req.json();

        
        const userRole = user?.extended_role || user?.role || 'user';
        if (userRole !== 'admin' && userRole !== 'ceo' && userRole !== 'hr_manager') {
            return Response.json({ error: 'Access denied. Must be admin, ceo, or hr_manager' }, { status: 403 });
        }
    
        console.log('[createSalarySnapshots] Params:', { calendar_period_id, report_run_id });

        if (!calendar_period_id || !report_run_id) {
            return Response.json({
                error: 'calendar_period_id and report_run_id are required'
            }, { status: 400 });
        }
        ...
        let divisor = settings?.salary_divisor || period.salary_calculation_days || 30;
        ...
        let otDivisor = settings?.ot_divisor || period.ot_calculation_days || divisor;
        if (!otDivisor || otDivisor <= 0) {
            console.warn('[createSalarySnapshots] Invalid ot_divisor from settings, using default divisor');
            otDivisor = divisor;
        }
        const isAlMaraghi = period.company === 'Al Maraghi Motors';
        ...
        if (isAlMaraghi) {
            const projectDateTo = new Date(period.date_to);
            ...
            const projectDateFrom = new Date(period.date_from);
            ...
            if (projectDateFrom < salaryMonthStart) {
                extraPrevMonthFrom = period.date_from;
        ...
        const reports = await base44.asServiceRole.entities.ReportRun.filter({ id: report_run_id, calendar_period_id: calendar_period_id }, null, 5000);
        if (reports.length === 0) {
        ...
        const [employees, salaries, analysisResults, allExceptions, salaryIncrements, rulesData, punches, shifts, allOvertimeData] = await Promise.all([
            ...
            base44.asServiceRole.entities.EmployeeSalary.filter({ company: period.company, active: true }, null, 5000),
            base44.asServiceRole.entities.AnalysisResult.filter({
                calendar_period_id: calendar_period_id,
                report_run_id: report_run_id
            }, null, 5000),
            base44.asServiceRole.entities.Exception.filter({ calendar_period_id: calendar_period_id }, null, 5000),
            isAlMaraghi
            ...
            base44.asServiceRole.entities.Punch.filter({ calendar_period_id: calendar_period_id }, null, 5000),
            base44.asServiceRole.entities.ShiftTiming.filter({ calendar_period_id: calendar_period_id }, null, 5000),
            base44.asServiceRole.entities.OvertimeData.filter({ calendar_period_id: calendar_period_id }, null, 5000)
        ]);
        ...
        const recalculateEmployeeAttendance = (emp, dateFrom, dateTo, assumedDays = []) => {
            ...
            const carriedGrace = period.use_carried_grace_minutes ? (emp.carried_grace_minutes || 0) : 0;
        ...
        // - Divisor: Uses OT Divisor (period.ot_calculation_days)
        ...
        const calculateExtraPrevMonthData = (emp, graceMinutes, prevMonthSalaryAmount, workingHours) => {
            ...
            const isJanuaryAlMaraghiProject = period.name === 'January - Al Maraghi Motors' ||
                period.name === 'January – Al Maraghi Motors';
            ...
            // Divisor = period.ot_calculation_days (NOT calendar days)
        ...
        const isAlMaraghiOrNaser = period.company === 'Al Maraghi Motors' ||
            period.company === 'Naser Mohsin Auto Parts';

        const shouldIncludeAllEmployees = isAlMaraghiOrNaser || includeAllEmployeesInSalary;

        console.log(`[createSalarySnapshots] ============================================`);
        console.log(`[createSalarySnapshots] COMPANY SALARY MODE: ${period.company}`);
        ...
        if (period.custom_employee_ids && period.custom_employee_ids.trim()) {
            const customIds = period.custom_employee_ids.split(',').map(id => id.trim()).filter(id => id);
        ...
        for (const emp of employeesToProcess) {
            ...
            const salaryLeaveDaysOverride = calculateSalaryLeaveDaysOverride(emp, period.date_from, period.date_to);
            ...
            const calculateWpsSplit = (totalAmount, capAmount, capEnabled) => {
                ...
                if (!(period.company === 'Al Maraghi Motors' && capEnabled)) {
                    return {
            ...
            snapshots.push({
                calendar_period_id: String(calendar_period_id),
                project_id: null,
