/**
 * PeriodOvertimeTab — Calendar-based equivalent of OvertimeTab.
 *
 ...
 * All existing overtime, bonus, incentive, salary adjustment,
 * and open leave salary logic are preserved exactly.
 */
...
export default function PeriodOvertimeTab({ calendarPeriod }) {
    ...
    const { data: employees = [], isLoading: loadingEmployees } = useQuery({
        ...
        queryFn: () => base44.entities.Employee.filter({ company: calendarPeriod.company, active: true }),
    ...
    const { data: overtimeRecords = [], isLoading: loadingOT, refetch: refetchOT } = useQuery({
        queryKey: ['overtimeData', calendarPeriod?.id],
        queryFn: () => base44.entities.OvertimeData.filter({ calendar_period_id: calendarPeriod.id }),
    ...
    const currentProjectCompanyId = useMemo(() => {
        return companies.find(company => company.name === calendarPeriod?.company)?.company_id ?? null;
    }, [companies, calendarPeriod?.company]);
    ...
    const isProjectClosed = calendarPeriod?.status === 'closed';
    ...
    const overtimeData = useMemo(() => {
        ...
        if (calendarPeriod?.custom_employee_ids) {
            const customIds = calendarPeriod.custom_employee_ids.split(',').map(id => id.trim()).filter(id => id);
    ...
    }, [employees, overtimeRecords, editableData, editableAdjustments, salarySnapshots, calendarPeriod?.custom_employee_ids, isAlMaraghiMotors]);
    ...
    const executeDownload = () => {
        ...
        try {
            ...
            XLSX.writeFile(wb, `OT_Template_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    const handleSyncToChecklist = async () => {
        ...
        try {
            ...
            if (otEmployees.length > 0) {
                const otTaskName = `Overtime Management - ${calendarPeriod.name}`;
                const otDescription = otEmployees.map(e => e.name).join(', ');
                const otFingerprint = `OT_MANAGEMENT_${calendarPeriod.id}`;
            ...
            for (const col of columns) {
                ...
                if (entries.length > 0) {
                    ...
                    const adjFingerprint = `ADJ_${col.key}_${calendarPeriod.id}`;
    ...
    return (
        ...
        <div className="mt-6">
            ...
            <ExcelPreviewDialog
                ...
                fileName={`OT_Template_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`}
