/**
 * PeriodExceptionsTab — Calendar-based equivalent of ExceptionsTab.
 *
 * Part of the new calendar-based payroll system.
 * Completely independent from the existing project system.
 * Replaces project_id with calendar_period_id.
 * When fetching exceptions, filters by calendar_period_id if available,
 * otherwise falls back to filtering by company and date range.
 * All existing exception management UI and logic are preserved exactly.
 */
...
function ChecklistSection({ calendarPeriod, checklistItems = [] }) {
    ...
    const initializePredefinedTasksMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries(['checklistItems', calendarPeriod.id]);
    ...
    const executeChecklistDownload = async () => {
        ...
        try {
            ...
            XLSX.writeFile(workbook, `Checklist_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    return (
        <Card className="border-0 shadow-sm bg-green-50/30">
            ...
            <ExcelPreviewDialog
                ...
                fileName={`Checklist_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`}
...
export default function PeriodExceptionsTab({ calendarPeriod }) {
    ...
    const { data: allExceptions = [] } = useQuery({
        ...
        queryFn: () => (async () => {
            // Calendar-based exception fetching: prefer calendar_period_id, fallback to company + date range
            if (calendarPeriod.id) {
                const byPeriod = await base44.entities.Exception.filter({ calendar_period_id: calendarPeriod.id });
                if (byPeriod.length > 0) return byPeriod;
            }
            // Fallback: filter by company and date range for backward compatibility
            const byCompany = await base44.entities.Exception.filter({ company: calendarPeriod.company });
            return byCompany.filter(ex => {
                if (!ex.date_from || !ex.date_to) return false;
                return ex.date_from <= calendarPeriod.date_to && ex.date_to >= calendarPeriod.date_from;
            });
        })()
    });
    ...
    const { data: currentUser } = useQuery({
     ...
     const { data: checklistItems = [], isLoading: checklistLoading } = useQuery({
        queryKey: ['checklistItems', calendarPeriod.id],
        queryFn: () => base44.entities.ChecklistItem.filter({ calendar_period_id: calendarPeriod.id }, 'created_date'),
        staleTime: 5 * 60 * 1000,
    ...
    const sortedChecklistItems = React.useMemo(() => {
        ...
        return [...checklistItems].sort((a, b) => {
            ...
            // Secondary Sort: Description/Name
...
456,Alice Brown,2025-11-21,2025-11-21,Half Day,Left early,0`;
    ...
    const handleExport = async () => {
        ...
        setPreviewConfig({
            ...
            fileName: `Exceptions_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`,
...
Project date range: ${calendarPeriod.date_from} to ${calendarPeriod.date_to}
...
Only include relevant fields. Match employee names/IDs intelligently.`,
    ...
    const handleSubmit = (e) => {
        ...
        // Clean up empty string values and convert early_checkout_minutes to number
         ...
         const cleanedData = {
            ...
            date_to: submitData.type === 'SINGLE_SHIFT' ? calendarPeriod.date_to : submitData.date_to,
    ...
    const sortedExceptions = React.useMemo(() => {
        return [...filteredExceptions].sort((a, b) => {
            ...
            const nameA = (employees.find(e => String(e.attendance_id) === String(a.attendance_id) && e.company === calendarPeriod.company)?.name || '').toLowerCase();
            const nameB = (employees.find(e => String(e.attendance_id) === String(b.attendance_id) && e.company === calendarPeriod.company)?.name || '').toLowerCase();
    ...
    }, [filteredExceptions, employees, calendarPeriod.company]);
    ...
    const handleGroupExport = async () => {
        ...
        const combinedData = [
            ...filteredExceptionsForExport.map(ex => {
                const employee = employees.find(e => String(e.attendance_id) === String(ex.attendance_id) && e.company === calendarPeriod.company);
        ...
        setPreviewConfig({
            ...
            fileName: `Unified_Export_${calendarPeriod.name}_${new Date().toISOString().split('T')[0]}.xlsx`,
                                       ...
                                       <Label>From Date <span className="text-red-500">*</span></Label>
                                        <Input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (newDate >= calendarPeriod.date_from && newDate <= calendarPeriod.date_to) {
                                                    setFormData(prev => {
                                    ...
                                    <div>
                                       <Label>To Date <span className="text-red-500">*</span></Label>
                                        <Input
                                            ...
                                            onChange={(e) => {
                                                const newDate = e.target.value;
                                                if (newDate >= formData.date_from && newDate <= calendarPeriod.date_to && newDate >= calendarPeriod.date_from) {
                                                    setFormData(prev => {
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 p-4 bg-slate-50 rounded-lg border">
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            const newDate = e.target.value;
                                            if (!newDate || (newDate >= calendarPeriod.date_from && newDate <= calendarPeriod.date_to)) {
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => {
                                            const newDate = e.target.value;
                                            if (!newDate || (newDate >= calendarPeriod.date_from && newDate <= calendarPeriod.date_to && (!filter.dateFrom || newDate >= filter.dateFrom))) {
                            ...
                            <Table>
                                ...
                                <TableBody>
                                   {paginatedExceptions.map((exception) => {
                                       const employeeName = employees.find(e => String(e.attendance_id) === String(exception.attendance_id) && e.company === calendarPeriod.company)?.name || '—';
                                       return (
                                           ...
                                           {!isUser && (
                                             ...
                                             <TableCell className="p-1 text-sm">
                                                 {exception.is_custom_type && (!exception.date_from || exception.date_from === calendarPeriod.date_from) 
                                                     ? '—' 
                                                     : new Date(exception.date_from).toLocaleDateString()}
                                             </TableCell>
                                             <TableCell className="p-1 text-sm">
                                                 {exception.is_custom_type && (!exception.date_to || exception.date_to === calendarPeriod.date_to) 
                                                     ? '—' 
                                ...
                                <TableBody>
                                    {filteredReportExceptions.length === 0 ? (
                                        ...
                                        <TableRow key={exception.id}>
                                            ...
                                            <TableCell className="p-1">
                                                <span className="text-sm text-slate-900">
                                                   {exception.attendance_id === 'ALL' ? '—' : (employees.find(e => String(e.attendance_id) === String(exception.attendance_id) && e.company === calendarPeriod.company)?.name || '—')}
                                                </span>
                                                </TableCell>
                                                <TableCell className="p-1">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-slate-900">
                                        {viewingException.attendance_id === 'ALL' 
                                            ? '—' 
                                            : employees.find(e => String(e.attendance_id) === String(viewingException.attendance_id) && e.company === calendarPeriod.company)?.name || '—'}
                                    </p>
                                </div>
                                <div>
