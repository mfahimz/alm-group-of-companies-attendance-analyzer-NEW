/**
 * PeriodRunAnalysisTab — Calendar-based equivalent of RunAnalysisTab.
 *
 ...
 * runCalendarAnalysis instead of runAnalysis.
 * All existing UI elements, progress indicators, error handling,
 * and display logic are preserved exactly.
 */
...
export default function PeriodRunAnalysisTab({ calendarPeriod }) {
    ...
    const [dateFrom, setDateFrom] = useState(calendarPeriod.date_from);
    const [dateTo, setDateTo] = useState(calendarPeriod.date_to);
    ...
    const { data: rules } = useQuery({
        queryKey: ['rules', calendarPeriod.company],
        queryFn: async () => {
            const rulesList = await base44.entities.AttendanceRules.filter({ company: calendarPeriod.company });
    ...
    const updateProjectMutation = useMutation({
        mutationFn: (status) => base44.entities.CalendarPeriod.update(calendarPeriod.id, { status }),
        onSuccess: () => {
            queryClient.invalidateQueries(['calendarPeriod', calendarPeriod.id]);
    ...
    const analyzeEmployee = async (attendance_id) => {
        ...
        const includeSeconds = calendarPeriod.company === 'Al Maraghi Automotive';
        ...
        for (let d = new Date(startDate); d <= endDate; d = new Date(d.setDate(d.getDate() + 1))) {
            ...
            try {
                if (calendarPeriod.company === 'Al Maraghi Auto Repairs' &&
                    dateException &&
            ...
            if (hasManualTimeException) {
                ...
                if (calendarPeriod.company === 'Al Maraghi Auto Repairs' && approvedMinutesForDay > 0) {
        ...
        const carriedGrace = calendarPeriod.use_carried_grace_minutes ? (employee?.carried_grace_minutes || 0) : 0;
        ...
        return {
            ...
            grace_minutes: (calendarPeriod.company === 'Al Maraghi Auto Repairs' || calendarPeriod.company === 'Al Maraghi Motors') ? (baseGrace * working_days) + carriedGrace : baseGrace + carriedGrace,
    ...
    const performDataQualityCheck = () => {
        ...
        if (duplicateHrms.length > 0) {
            issues.push({
                type: 'error',
                title: `Duplicate HRMS IDs found in ${calendarPeriod.company}`,
        ...
        if (duplicateAtt.length > 0) {
            issues.push({
                type: 'error',
                title: `Duplicate Attendance IDs found in ${calendarPeriod.company}`,
        ...
        Object.values(punchsByDate).forEach(dayPunches => {
            if (dayPunches.length > 1) {
                const sorted = dayPunches.sort((a, b) => {
                    const timeA = parseTime(a.timestamp_raw, calendarPeriod.company === 'Al Maraghi Automotive');
                    const timeB = parseTime(b.timestamp_raw, calendarPeriod.company === 'Al Maraghi Automotive');
                    return (timeA?.getTime() || 0) - (timeB?.getTime() || 0);
                });

                for (let i = 1; i < sorted.length; i++) {
                    const prevTime = parseTime(sorted[i - 1].timestamp_raw, calendarPeriod.company === 'Al Maraghi Automotive');
                    const currTime = parseTime(sorted[i].timestamp_raw, calendarPeriod.company === 'Al Maraghi Automotive');
        ...
        punches.forEach(p => {
            if (p.punch_date >= dateFrom && p.punch_date <= dateTo) {
                ...
                punchesMap[key] = (punchesMap[key] || 0) + 1;
            }
        });
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            while (!isComplete) {
                const response = await base44.functions.invoke('runCalendarAnalysis', {
            ...
            queryClient.invalidateQueries(['results', calendarPeriod.id]);
            queryClient.invalidateQueries(['reportRuns', calendarPeriod.id]);
            queryClient.invalidateQueries(['calendarPeriod', calendarPeriod.id]);
            queryClient.invalidateQueries(['calendarPeriods']);
