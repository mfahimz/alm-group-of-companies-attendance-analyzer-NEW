// BodyShopPayrollTab.jsx
// Exact replica of Salary Report table — Body Shop employees only (department === 'Bodyshop')
...
export default function BodyShopPayrollTab({
    salaryData = [],
    adminEditMode = false,
    setAdminEditMode,
    editableData = {},
    ...
    const bodyShopEmployees = useMemo(() => {
        return salaryData.filter(row => row.department === 'Bodyshop');
    }, [salaryData]);

    // Filter body shop employees by search query then sort by sortColumn
    // Mirrors the exact sort logic used in SalaryReportDetail.jsx
    const filteredData = useMemo(() => {
        const query = (searchQuery || '').toLowerCase();
        const filtered = bodyShopEmployees.filter(row =>
            !searchQuery ||
    ...
    }, [bodyShopEmployees, searchQuery, sortColumn]);

    const grandTotals = useMemo(() => {
        return bodyShopEmployees.reduce((acc, row) => {
                                    ...
                                    <td className={`${cellBase} px-1 sticky left-0 z-10 ${stickyStripe} shadow-[2px_0_4px_-2px_rgba(0,0,0,0.05)]`}>
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                {saveErrors[String(row.attendance_id)] && (
                                                    <span className="text-[10px] leading-tight text-red-500 font-medium text-center">
                                                        {saveErrors[String(row.attendance_id)]}
                                                    </span>
                                                )}
                                            </div>
